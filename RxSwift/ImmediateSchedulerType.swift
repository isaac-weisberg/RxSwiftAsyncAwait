...
public extension ImmediateSchedulerType {
    /**
     ...
     - returns: The disposable object used to cancel the scheduled action (best effort).
     */
    func scheduleRecursive<State>(_ state: State, action: @escaping (_ state: State, _ recurse: (State) async -> Void) async -> Void) async -> Disposable {
        let recursiveScheduler = await RecursiveImmediateScheduler(action: action, scheduler: self)
