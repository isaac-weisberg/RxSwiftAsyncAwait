...
public extension ObservableType {
     ...
     */
    func subscribe(_ lock: ActorLock, _ on: @escaping (Event<Element>) -> Void) -> Disposable {
        let observer = AnonymousObserver { e in
            on(e)
        }
        return asObservable().subscribe(lock, observer)
    }
    ...
    func subscribe(
        ...
        return Disposables.create(
            asObservable().subscribe(lock, observer),
            disposable
        )
    }
}
