...
public final actor AsyncSubject<Element: Sendable>:
    ObservableType,
    SubjectType,
    ObserverType,
    ...
    public func on(_ event: Event<Element>, _ c: C) async {
        let (observers, event) = Asynchronous_on(event, c.call())
        switch event {
        case .next:
    ...
    func Asynchronous_on(_ event: Event<Element>, _ c: C) -> (Observers, Event<Element>) {
        if isStopped {
    ...
    public nonisolated func asObserver() -> AsyncSubject<Element> {
        self
    }
