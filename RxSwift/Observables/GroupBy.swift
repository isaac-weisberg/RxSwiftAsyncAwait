...
private final class GroupedObservableImpl<Element>: Observable<Element> {
    ...
    override public func subscribe<Observer: ObserverType>(_ lock: ActorLock, _ observer: Observer) -> Disposable
        where Observer.Element == Element {
        let release = refCount.retain()
        let subscription = subject.subscribe(lock, observer)
...
private final class GroupBySink<Key: Hashable, Element, Observer: ObserverType>:
    ...
    func run(_ lock: ActorLock) -> Disposable {
        ...
        subscription.setDisposable(parent.source.subscribe(lock, self))
...
private final class GroupBy<Key: Hashable, Element>: Producer<GroupedObservable<Key, Element>> {
    ...
    override func run<Observer: ObserverType>(
        ...
        _ observer: Observer,
        cancel: Cancelable
    ) -> (sink: Disposable, subscription: Disposable) where Observer.Element == GroupedObservable<
        Key,
        Element
    > {
        let sink = GroupBySink(parent: self, observer: observer, cancel: cancel)
        return (sink: sink, subscription: sink.run(lock))
    }
}
