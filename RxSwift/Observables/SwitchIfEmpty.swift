...
private final class SwitchIfEmptySink<Observer: ObserverType>: Sink<Observer>,
    ...
    init(lock: ActorLock, ifEmpty: Observable<Element>, observer: Observer, cancel: Cancelable) {
        self.ifEmpty = ifEmpty
        self.lock = lock
    ...
    func run(_ source: Observable<Observer.Element>) -> Disposable {
        let subscription = source.subscribe(lock, self)
    ...
    func on(_ event: Event<Element>) {
        ...
        case .completed:
            ...
            ifEmptySubscription.setDisposable(ifEmpty.subscribe(lock, ifEmptySink))
        }
    }
}
