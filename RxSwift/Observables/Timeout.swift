...
private final class TimeoutSink<Observer: ObserverType>: Sink<Observer>, LockOwnerType, ObserverType {
    ...
    init(_ lock: ActorLock, parent: Parent, observer: Observer, cancel: Cancelable) {
        actorLock = lock
        self.parent = parent
    ...
    func run() -> Disposable {
        ...
        original.setDisposable(parent.source.subscribe(actorLock, self))
    ...
    private func createTimeoutTimer() {
        ...
        let disposeSchedule = parent.scheduler.scheduleRelative(id, dueTime: parent.dueTime) { state in
            ...
            if timerWins {
                self.subscription.disposable = self.parent.other.subscribe(self.actorLock, self.forwarder())
            }
...
private final class Timeout<Element>: Producer<Element> {
    ...
    override func run<Observer: ObserverType>(
        ...
        let sink = TimeoutSink(lock, parent: self, observer: observer, cancel: cancel)
