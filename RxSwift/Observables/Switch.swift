...
private class SwitchSink<SourceType, Source: ObservableConvertibleType, Observer: ObserverType>:
    ...
    init(_ lock: ActorLock, observer: Observer, cancel: Cancelable) {
        self.actorLock = lock
    ...
    func on(_ event: Event<Element>) {
        ...
        case .next(let element):
            if let (latest, observable) = nextElementArrived(element: element) {
                ...
                let disposable = observable.subscribe(actorLock, observer)
...
private final class SwitchIdentitySink<Source: ObservableConvertibleType, Observer: ObserverType>: SwitchSink<
    Source,
    Source,
    Observer
>
...
private final class MapSwitchSink<SourceType, Source: ObservableConvertibleType, Observer: ObserverType>: SwitchSink<
    SourceType,
    Source,
    Observer
> where Observer.Element == Source.Element {
    ...
    init(_ lock: ActorLock, selector: @escaping Selector, observer: Observer, cancel: Cancelable) {
        ...
        super.init(lock, observer: observer, cancel: cancel)
    }
...
private final class Switch<Source: ObservableConvertibleType>: Producer<Source.Element> {
    ...
    override func run<Observer: ObserverType>(
        ...
        _ observer: Observer,
        cancel: Cancelable
    ) -> (sink: Disposable, subscription: Disposable) where Observer.Element == Source.Element {
        let sink = SwitchIdentitySink<Source, Observer>(lock, observer: observer, cancel: cancel)
...
private final class FlatMapLatest<SourceType, Source: ObservableConvertibleType>: Producer<Source.Element> {
    ...
    override func run<Observer: ObserverType>(
        ...
        let sink = MapSwitchSink<SourceType, Source, Observer>(lock, selector: selector, observer: observer, cancel: cancel)
        let subscription = sink.run(source)
        return (sink: sink, subscription: subscription)
    }
}
