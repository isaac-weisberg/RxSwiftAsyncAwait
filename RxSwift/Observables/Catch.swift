...
public extension ObservableType {
    ...
    func catchError(_ handler: @escaping (Swift.Error) async throws -> Observable<Element>)
        -> Observable<Element> {
        self.catch(handler)
    }
...
private final class CatchSinkProxy<Observer: ObserverType>: ObserverType {
    ...
    func on(_ event: Event<Element>, _ c: C) async {
        await parent.handleCatchEvent(event, c.call())
    }
}
...
private final actor CatchSink<Observer: ObserverType>: Sink, ObserverType {
    ...
    private let catchSubscription = SingleAssignmentDisposable()
    ...
    func on(_ event: Event<Element>, _ c: C) async {
        ...
        case .error(let error):
            do {
                ...
                await catchSubscription.setDisposable(catchSequence.subscribe(c.call(), observer))?.dispose()
            } catch let e {
    ...
    func handleCatchEvent(_ event: Event<Element>, _ c: C) async {
        if baseSink.disposed {
            return
        }
        await forwardOn(event, c.call())
    ...
    func dispose() async {
        ...
        async let sourceDispose: ()? = sourceSubscription.dispose()?.dispose()
        async let catchDispose: ()? = catchSubscription.dispose()?.dispose()
...
//// catch enumerable
//
// private final actor CatchSequenceSink<Sequence: Swift.Sequence, Observer: ObserverType>: ObserverType,
// TailRecursiveSinkUser
