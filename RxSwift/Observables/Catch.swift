...
private final class CatchSink<Observer: ObserverType>: Sink<Observer> {
    typealias Element = Observer.Element
    ...
    func run(_ lock: ActorLock) -> Disposable {
        ...
        let observer = AnyObserver(eventHandler: { [self] event in
            on(lock, event)
        })
        d1.setDisposable(parent.source.subscribe(lock, observer))
    ...
    func on(_ lock: ActorLock, _ event: Event<Element>) {
        ...
        case .error(let error):
            do {
                ...
                subscription.disposable = catchSequence.subscribe(lock, observer)
            } catch let e {
...
private final class CatchSequenceSink<Sequence: Swift.Sequence, Observer: ObserverType>:
    ...
    ObserverThatCapturesLock where Sequence.Element: ObservableConvertibleType,
...
private final class CatchSequence<Sequence: Swift.Sequence>: Producer<Sequence.Element.Element>
    ...
    override func run<Observer: ObserverType>(
        ...
        let subscription = sink.run(lock, (sources.makeIterator(), nil))
        return (sink: sink, subscription: subscription)
    }
}
