...
>: MergeLimitedSink<SourceElement, SourceSequence, Observer> where Observer.Element == SourceSequence.Element {
    ...
    init(_ lock: ActorLock, selector: @escaping Selector, observer: Observer, cancel: Cancelable) {
        self.selector = selector
        super.init(lock, maxConcurrent: 1, observer: observer, cancel: cancel)
    }
...
private class MergeLimitedSink<SourceElement, SourceSequence: ObservableConvertibleType, Observer: ObserverType>:
    ...
    init(_ actorLock: ActorLock, maxConcurrent: Int, observer: Observer, cancel: Cancelable) {
        self.actorLock = actorLock
    ...
    func run(_ source: Observable<SourceElement>) -> Disposable {
        ...
        let disposable = source.subscribe(actorLock, self)
    ...
    func subscribe(_ innerSource: SourceSequence, group: CompositeDisposable) {
        ...
        if let key {
            ...
            let disposable = innerSource.asObservable().subscribe(actorLock, observer)
...
private final class MergeLimited<SourceSequence: ObservableConvertibleType>: Producer<SourceSequence.Element> {
    ...
    override func run<Observer: ObserverType>(
        ...
        let sink = MergeLimitedBasicSink<SourceSequence, Observer>(
            lock,
...
>: MergeSink<SourceElement, SourceSequence, Observer> where Observer.Element == SourceSequence.Element {
    ...
    init(_ actorLock: ActorLock, selector: @escaping Selector, observer: Observer, cancel: Cancelable) {
        self.selector = selector
        super.init(actorLock, observer: observer, cancel: cancel)
    }
...
private class MergeSink<SourceElement, SourceSequence: ObservableConvertibleType, Observer: ObserverType>:
    ...
    init(_ actorLock: ActorLock, observer: Observer, cancel: Cancelable) {
        self.actorLock = actorLock
        super.init(observer: observer, cancel: cancel)
    }
    ...
    func subscribeInner(_ source: Observable<Observer.Element>) {
        ...
        if let disposeKey = group.insert(iterDisposable) {
            ...
            let subscription = source.subscribe(actorLock, iter)
    ...
    func run(_ source: Observable<SourceElement>) -> Disposable {
        ...
        let subscription = source.subscribe(actorLock, self)
...
>: Producer<SourceSequence.Element> {
    ...
    override func run<Observer: ObserverType>(
        ...
        where Observer.Element == SourceSequence.Element {
        let sink = FlatMapSink(lock, selector: selector, observer: observer, cancel: cancel)
        let subscription = sink.run(source)
        return (sink: sink, subscription: subscription)
    }
}
...
>: Producer<SourceSequence.Element> {
    ...
    override func run<Observer: ObserverType>(
        ...
        where Observer.Element == SourceSequence.Element {
        let sink = FlatMapFirstSink<SourceElement, SourceSequence, Observer>(
            lock,
            selector: selector,
            observer: observer,
            cancel: cancel
...
final class ConcatMap<SourceElement, SourceSequence: ObservableConvertibleType>: Producer<SourceSequence.Element> {
    ...
    override func run<Observer: ObserverType>(
        ...
        where Observer.Element == SourceSequence.Element {
        let sink = ConcatMapSink<SourceElement, SourceSequence, Observer>(
            lock,
            selector: selector,
            observer: observer,
            cancel: cancel
...
final class Merge<SourceSequence: ObservableConvertibleType>: Producer<SourceSequence.Element> {
    ...
    override func run<Observer: ObserverType>(
        ...
        where Observer.Element == SourceSequence.Element {
            let sink = MergeBasicSink<SourceSequence, Observer>(lock, observer: observer, cancel: cancel)
        let subscription = sink.run(source)
        return (sink: sink, subscription: subscription)
    }
}
...
private final class MergeArray<Element>: Producer<Element> {
    ...
    override func run<Observer: ObserverType>(
        ...
        let sink = MergeBasicSink<Observable<Element>, Observer>(lock, observer: observer, cancel: cancel)
