...
public extension PrimitiveSequenceType where Trait == MaybeTrait {
    typealias MaybeObserver = (MaybeEvent<Element>) async -> Void
     ...
     */
    static func create(subscribe: @escaping (@escaping MaybeObserver) -> Disposable)
        -> PrimitiveSequence<Trait, Element> {
        let source = Observable<Element>.createUnsynchronized { lock, observer in
            subscribe(actorLocked(lock) { event in
                switch event {
     ...
     */
    func subscribe(_ lock: ActorLock, _ observer: @escaping (MaybeEvent<Element>) -> Void) -> Disposable {
        var stopped = false
        return primitiveSequence.asObservable().subscribe(lock) { event in
            if stopped { return }
            stopped = true
    ...
    func subscribe(
        ...
        let observer: (MaybeEvent<Element>) -> Void = { event in
            switch event {
            case .success(let element):
                onSuccess?(element)
                disposable.dispose()
        ...
        return Disposables.create(
            primitiveSequence.subscribe(lock, observer),
            disposable
        )
    }
}
