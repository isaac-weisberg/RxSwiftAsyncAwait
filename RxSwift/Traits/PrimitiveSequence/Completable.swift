...
public extension PrimitiveSequenceType where Trait == CompletableTrait, Element == Swift.Never {
     ...
     */
    static func create(subscribe: @escaping (@escaping CompletableObserver) -> Disposable)
     ...
     */
    func subscribe(_ lock: ActorLock, _ observer: @escaping (CompletableEvent) -> Void) -> Disposable {
        var stopped = false
    ...
    func subscribe(
        ...
        let observer: (CompletableEvent) -> Void = { event in
            switch event {
            case .error(let error):
                if let onError {
                    onError(error)
